...
package com.gu.devel.sinaweibo.userinfo.appbar.widget.tablayout;
 ...
 *
 * <p>If you're using a {@link ViewPager} together with this layout, you can call {@link
 * #setupWithViewPager(ViewPager)} to link the two together. This layout will be automatically
 * populated from the {@link PagerAdapter}'s page titles.
 *
...
public class TabLayout extends HorizontalScrollView {
  ...
  public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    super.addView(
        mTabStrip, 0, new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
    ...
    TypedArray a =
        context.obtainStyledAttributes(
            ...
            defStyleAttr,
            android.support.design.R.style.Widget_Design_TabLayout);

    mTabStrip.setSelectedIndicatorHeight(
        a.getDimensionPixelSize(
            android.support.design.R.styleable.TabLayout_tabIndicatorHeight, 0));
    ...
    mTabPaddingStart =
        mTabPaddingTop =
            mTabPaddingEnd =
                mTabPaddingBottom =
                    a.getDimensionPixelSize(
                        android.support.design.R.styleable.TabLayout_tabPadding, 0);
    mTabPaddingStart =
        a.getDimensionPixelSize(
            android.support.design.R.styleable.TabLayout_tabPaddingStart, mTabPaddingStart);
    mTabPaddingTop =
        a.getDimensionPixelSize(
            android.support.design.R.styleable.TabLayout_tabPaddingTop, mTabPaddingTop);
    mTabPaddingEnd =
        a.getDimensionPixelSize(
            android.support.design.R.styleable.TabLayout_tabPaddingEnd, mTabPaddingEnd);
    mTabPaddingBottom =
        a.getDimensionPixelSize(
            android.support.design.R.styleable.TabLayout_tabPaddingBottom, mTabPaddingBottom);
    ...
    mRequestedTabMinWidth =
        a.getDimensionPixelSize(
            android.support.design.R.styleable.TabLayout_tabMinWidth, INVALID_WIDTH);
    mRequestedTabMaxWidth =
        a.getDimensionPixelSize(
            android.support.design.R.styleable.TabLayout_tabMaxWidth, INVALID_WIDTH);
    mTabBackgroundResId =
      ...
      if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
          ...
          if (child.getVisibility() == VISIBLE || child.getVisibility() == INVISIBLE) {
            minTabWidth = Math.max(minTabWidth, child.getMeasuredWidth());
          }
        }
        if (mSelectedIndicatorWidth > 0
            && getSelectedTabPosition() >= 0
            && (mIsIndicatorWidthWrapContent || (mSelectedIndicatorWidth <= minTabWidth))) {
          if (mViewPager == null || !mIsDragged) {
            left =
                mIndicatorLeft
                    + (getChildAt(getSelectedTabPosition()).getMeasuredWidth()
                            - mSelectedIndicatorWidth)
                        / 2.0f;
            right = left + mSelectedIndicatorWidth;
          }
          updateRect(mRect, left, getHeight() - mSelectedIndicatorHeight, right, getHeight());
          canvas.drawRoundRect(mRect, rd, rd, mSelectedIndicatorPaint);
          ...
          if (mScrollState == SCROLL_STATE_SETTLING && mPreviousScrollState == SCROLL_STATE_IDLE) {
            ...
            if (tabLayout.mTabStrip.mSelectedPosition + 1 < tabLayout.mTabStrip.getChildCount()) {
              float offset =
                  (tabLayout
                              .mTabStrip
                              .getChildAt(tabLayout.mTabStrip.mSelectedPosition)
                              .getMeasuredWidth()
                          - tabLayout.mTabStrip.getWrapContentIndicatorWidth(
                              tabLayout.mTabStrip.mSelectedPosition))
                      / 2.0f;
              float leftX =
                  tabLayout.mTabStrip.getChildAt(tabLayout.mTabStrip.mSelectedPosition).getLeft()
                      + offset;
              float rightX =
                  leftX
                      + tabLayout.mTabStrip.getWrapContentIndicatorWidth(
              ...
              float nextOffset =
                  (tabLayout
                              .mTabStrip
                              .getChildAt(tabLayout.mTabStrip.mSelectedPosition + 1)
                              .getMeasuredWidth()
                          - tabLayout.mTabStrip.getWrapContentIndicatorWidth(
                              tabLayout.mTabStrip.mSelectedPosition + 1))
                      / 2.0f;
              float nextLeftX =
                  tabLayout.mTabStrip.getChildAt(tabLayout.mTabStrip.mSelectedPosition).getLeft()
                      + tabLayout
  ...
  /**
   * A {@link OnTabSelectedListener} class which contains the necessary calls back to the provided
   * {@link ViewPager} so that the tab position is kept in sync.
   */
