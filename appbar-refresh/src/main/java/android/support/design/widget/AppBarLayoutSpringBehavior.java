...
public class AppBarLayoutSpringBehavior extends AppBarLayout.Behavior {
  ...
  public void onNestedScroll(
    ...
    if (dyUnconsumed < 0) {
      setHeaderTopBottomOffset(
          coordinatorLayout,
          child,
          getTopBottomOffsetForScrollingSibling() - dyUnconsumed,
  ...
  private void animateFlingSpring(
      final CoordinatorLayout coordinatorLayout, final AppBarLayout abl, int originNew) {
    if (mFlingAnimator == null) {
  ...
  private void animateRecoverBySpring(
  ...
  private void snapToChildIfNeeded(CoordinatorLayout coordinatorLayout, AppBarLayout abl) {
    ...
    if (offsetChildIndex >= 0) {
      ...
      final AppBarLayout.LayoutParams lp =
      ...
      if ((flags & AppBarLayout.LayoutParams.FLAG_SNAP) == AppBarLayout.LayoutParams.FLAG_SNAP) {
        ...
        if (checkFlag(flags, AppBarLayout.LayoutParams.SCROLL_FLAG_EXIT_UNTIL_COLLAPSED)) {
          ...
          snapBottom += ViewCompat.getMinimumHeight(offsetChild);
            ...
            flags,
            AppBarLayout.LayoutParams.FLAG_QUICK_RETURN
        ...
        animateOffsetTo(coordinatorLayout, abl, clamp(newOffset, -abl.getTotalScrollRange(), 0), 0);
      }
    }
  }
      ...
      mOffsetAnimator.addUpdateListener(
          new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animator) {
              setHeaderTopBottomOffset(
                  coordinatorLayout, child, (Integer) animator.getAnimatedValue());
            }
          });
    } else {
      mOffsetAnimator.cancel();
  ...
  int setHeaderTopBottomOffset(
    ...
    return setHeaderTopBottomOffset(
        coordinatorLayout, appBarLayout, newOffset, minOffset, maxOffset, -1);
  }
  ...
  int setHeaderTopBottomOffset(
    ...
    if (mOffsetSpring != 0 && newOffset < 0) {
      ...
      if (newSpringOffset >= 0) return consumed;
    }
    ...
    if (minOffset != 0 && curOffset >= minOffset && curOffset <= maxOffset) {
      ...
      if (curOffset != newOffset) {
        ...
        updateAppBarLayoutDrawableState(
            coordinatorLayout, appBarLayout, newOffset, newOffset < curOffset ? -1 : 1, false);
  ...
  private int updateSpringByScroll(
      CoordinatorLayout coordinatorLayout, AppBarLayout appBarLayout, int type, int originNew) {
    int consumed;
    if (appBarLayout.getHeight() >= mPreHeadHeight && type == 1) {
      //      if (mFlingAnimator == null) animateFlingSpring(coordinatorLayout, appBarLayout,
      // originNew);
      return originNew;
    }
  ...
  private void updateSpringOffsetByscroll(
  ...
  private void updateSpringHeaderHeight(
      CoordinatorLayout coordinatorLayout, AppBarLayout appBarLayout, int offset) {
